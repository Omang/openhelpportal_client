    ...
    <div className=' flex items-center justify-center mt-8'>
      <div className=' border border-green-500 rounded-2xl shadow-2xl w-[400px]'>
        {loading ? <GridLoader color={'#7ED321'} loading={loading} size={20} /> : <form className='flex flex-col p-2' onSubmit={registeruser}>
           ...
           <input required type="number" value={mobile} onChange={e=>setMobile(e.target.value)} placeholder='Your Mobile number' className='border border-green-500 rounded-full m-3 px-3' />
           <input required type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder='Enter password' className='border border-green-500 rounded-full m-3 px-3' />
           <select onChange={onchangeHandler} type="text"  className='border border-green-500 rounded-full m-3 px-3 '>
           ...
           <button type='submit' className='border rounded-full px-3 hover:text-white bg-green-500'>Register</button>
           <Link className='underline hover:text-green-600 text-red-600' to={'/login'}>Login</Link>
           </div>
        </form>}
      </div>
    </div>
