...
const Contractmain = ({peopletype}) => {
  ...
  const searchitem = async(e)=>{
    ...
    try{
      setLoading(true);
      setResult([]);
      const {data} = await axios.post('/ai/runcontract', {
        question: search
      })
      if(data.message !== ''){
        setLoading(false);
...
<button onClick={clearfield} className='bg-gray-500 text-white hover:bg-gray-200 hover:text-black px-3 m-4'>Clear</button>
           ...
           <form  className="flex flex-col" onSubmit={searchitem}>
            ...
            <div className='bg-gray-100 flex flex-col'>
                 ...
                 <p className='px-4 m-2 text-xs text-red-500'>Help write a contract between an artist and record label </p>
                 <p className='px-4 m-2 text-xs text-red-500'>I want to borrow money from someone, please draft a contract</p>
             </div>
           </div>
        </div>
